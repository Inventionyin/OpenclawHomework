...
ClawEmail 的 primary 主邮箱可以作为 SMTP From 发信身份，例如 `watchee@claw.163.com` 或 `shine1@claw.163.com`；下面这些 sub 子邮箱只作为 To / Archive / Role 收件地址或动作入口，不能作为 From 发信。

| 动作 | 邮箱 | 用途 |
| --- | --- | --- |
| `task` | `watchee.ui@claw.163.com` | 邮件调度入口，后续可通过邮件触发 UI 自动化 |
| `report` | `watchee.report@claw.163.com` | UI 自动化报告汇总 |
| `verify` | `evasan.account@claw.163.com` | 电商注册、验证码、账号验证 |
...
| `eval` | `agent4.archive@claw.163.com` | Agent 能力评测和模型对比归档 |
| `files` | `agent3.files@claw.163.com` | 截图、trace、video、artifact 链接 |
| `archive` | `agent4.archive@claw.163.com` | 训练语料、失败样本、复盘归档 |
...
| 邮箱 | 用途 | 是否适合注册测试 |
| --- | --- | --- |
| `evasan.account@claw.163.com` | 注册、登录、找回密码验证码 | 适合 |
...
| `agent4.archive@claw.163.com` | Agent 评测和模型对比 | 只做内部评测归档 |
| `agent4.archive@claw.163.com` | 失败样本、训练语料、复盘 | 只做归档 |
...
- 把失败样本归档到 `agent4.archive@claw.163.com`。
...
用 `evasan.account@claw.163.com` 作为电商平台注册/找回密码测试邮箱。
...
1. 邮件监听器：收到 `watchee.ui@claw.163.com` 邮件后自动触发 GitHub Actions。
