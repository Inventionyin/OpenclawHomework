...
test('buildEmailPlaybook binds mailbox addresses to real QA actions', () => {
  ...
  assert(playbook.some((item) => item.mailbox === 'watchee.ui@claw.163.com' && /触发/.test(item.action)));
  assert(playbook.some((item) => item.mailbox === 'evasan.account@claw.163.com' && /验证码/.test(item.action)));
  assert(playbook.some((item) => item.mailbox === 'agent4.archive@claw.163.com' && /评测/.test(item.action)));
});
